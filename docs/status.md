...
The [M2 acceptance gap index](m2-acceptance-gaps.md) maps each task to evidence and outstanding work. Historical IAM token-field feasibility and QS FIN-loss through the original consumer/gRPC/business boundary now have direct proofs. Graceful broker restart preserved all 10 original messages. A separate SIGKILL characterization lost all 10 confirmed memory-only messages despite retaining the volume; this is an observed durability limit, not successful crash recovery. See the candidate API/behavior review in [M2 review](m2-review.md).
